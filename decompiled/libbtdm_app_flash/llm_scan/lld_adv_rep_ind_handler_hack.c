/*
 * Last changed at upstream commit c57c0b11c3c0065a16b66685715100a189ef9b27
 * https://github.com/espressif/esp32c3-bt-lib/commit/c57c0b11c3c0065a16b66685715100a189ef9b27
 * Upstream date: 2024-12-13 13:39:25 +0800
 * Upstream subject: fix(bt): Update bt lib for ESP32-C3 and ESP32-S3(555b0a2)
 * Source: libbtdm_app_flash -> llm_scan.o -> lld_adv_rep_ind_handler_hack
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

undefined4 lld_adv_rep_ind_handler_hack(char *param_1)

{
  char cVar1;
  undefined1 uVar2;
  undefined2 uVar3;
  byte bVar4;
  uint uVar5;
  uint uVar6;
  int iVar7;
  void *pvVar8;
  undefined1 *puVar9;
  int iVar10;
  undefined2 *puVar11;
  int iVar12;
  byte bVar13;
  ushort uVar14;
  ushort uVar15;
  byte bVar16;
  undefined1 uVar17;
  ushort uVar18;
  uint __n;
  byte *pbVar19;
  ushort uStack_62;
  undefined4 uStack_60;
  undefined2 uStack_5c;
  undefined4 uStack_58;
  undefined2 uStack_54;
  undefined4 uStack_50;
  undefined2 uStack_4c;
  undefined4 uStack_48;
  undefined2 uStack_44;
  
  iVar12 = r_sdk_config_get_opts_ext();
  if (*(char *)(iVar12 + 0x23) == '\0') {
    return 0;
  }
  bVar13 = (&co_rate_to_phy)[(byte)param_1[0x19]];
  uVar18 = (ushort)(byte)param_1[0x17];
  uStack_58 = 0;
  uStack_54 = 0;
  uStack_50 = 0;
  uStack_4c = 0;
  uStack_48 = 0;
  pbVar19 = *(byte **)(*(int *)(_p_llm_env + 8) + (uint)(byte)param_1[0x14] * 0x44);
  uStack_44 = 0;
  uStack_60 = 0;
  uStack_5c = 0;
  bVar4 = 0;
  if (uVar18 != 0xff) {
    uStack_62 = *(ushort *)(param_1 + 2);
    uVar6 = 0;
    if (((((param_1[0x15] & 4U) != 0) && (uVar6 = (uint)(byte)param_1[0x13], uVar6 != 0)) &&
        (uVar6 = 0, *param_1 == '\0')) && (uVar6 = (uint)(byte)param_1[1], uVar6 != 0)) {
      uVar6 = lld_priv_rpa_res(param_1 + 4,param_1 + 10,&uStack_62);
    }
    if (uStack_62 == 0) {
      if (uVar6 != 0) {
        uVar5 = 0;
        goto _L556;
      }
_L511:
      memcpy(&uStack_60,param_1 + 4,6);
    }
    else {
      uVar5 = (uStack_62 - 0xc60) / 0x34 & 0xff;
      if (uVar6 != 0) {
_L556:
        iVar12 = r_emi_get_mem_addr_by_offset(0xc60);
        uVar18 = *(ushort *)(iVar12 + uVar5 * 0x34);
        memcpy(&uStack_50,param_1 + 4,6);
        uVar18 = uVar18 & 1 | 2;
        if (uStack_62 == 0) goto _L511;
      }
      iVar7 = r_emi_get_mem_addr_by_offset(0xc60);
      iVar12 = uVar5 * 0x34;
      if ((*(ushort *)(iVar7 + iVar12) >> 1 & 1) != 0) {
        pvVar8 = (void *)r_emi_get_mem_addr_by_offset(uStack_62 + 0x18);
        memcpy(&uStack_60,pvVar8,6);
      }
      iVar7 = r_emi_get_mem_addr_by_offset(0xc60);
      if ((*(ushort *)(iVar7 + iVar12) >> 7 & 1) != 0) {
        pvVar8 = (void *)r_emi_get_mem_addr_by_offset(uStack_62 + 0x2e);
        memcpy(&uStack_58,pvVar8,6);
      }
      if ((*param_1 != '\0') || ((param_1[0x15] & 6U) != 4)) {
        iVar7 = r_emi_get_mem_addr_by_offset(0xc60);
        if ((*(ushort *)(iVar7 + iVar12) >> 3 & 1) != 0) {
          pvVar8 = (void *)r_emi_get_mem_addr_by_offset(uStack_62 + 0x12);
          memcpy(&uStack_50,pvVar8,6);
        }
        iVar7 = r_emi_get_mem_addr_by_offset(0xc60);
        if (((*(ushort *)(iVar7 + iVar12) >> 3 & 1) != 0) ||
           (iVar7 = r_emi_get_mem_addr_by_offset(0xc60), (*(ushort *)(iVar7 + iVar12) >> 7 & 1) != 0
           )) {
          iVar7 = r_emi_get_mem_addr_by_offset(0xc60);
          uVar18 = *(ushort *)(iVar7 + iVar12) & 1;
          if ((param_1[9] & 0xc0U) == 0x40) {
            uVar18 = uVar18 | 2;
          }
        }
      }
    }
    if ((param_1[0x15] & 4U) == 0) {
_L524:
      bVar4 = 0;
    }
    else if (param_1[1] == '\0') {
      memcpy(&uStack_48,param_1 + 10,6);
      bVar4 = param_1[0x18];
      if (bVar4 == 0) goto _L524;
      if ((param_1[0xf] & 0xc0U) != 0xc0) {
        bVar4 = 0xfe;
      }
    }
    else {
      bVar4 = *pbVar19 & 1;
      if ((param_1[0xf] & 0xc0U) == 0x40) {
        bVar4 = bVar4 | 2;
      }
      memcpy(&uStack_48,(void *)(_p_llm_env + 0xc),6);
    }
  }
  iVar12 = r_llm_adv_rep_flow_control_check_eco(param_1,&uStack_60);
  if (iVar12 == 0) {
    if ((bVar13 & 0xfd) != 1) {
      r_assert_err("llm_scan.c",0x730);
    }
    if (((((param_1[0x15] & 4U) == 0) || ((uStack_44._1_1_ & 0xc0) != 0x40)) || (bVar4 != 0xfe)) ||
       (1 < pbVar19[1])) {
      bVar16 = *(byte *)(_p_llm_env + 0xd4);
      if ((bVar16 & 1) != 0) {
        if (bVar13 == 1) {
          uVar6 = (int)(uint)bVar16 >> 2;
        }
        else {
          uVar6 = (int)(uint)bVar16 >> 4;
        }
        iVar12 = 0;
        if ((uVar6 & 3) == 0) {
          iVar10 = r_llm_le_scan_duplicate_opt_get();
          iVar7 = _p_llm_env;
          if (((*(char *)(_p_llm_env + 0xd7) == '\x02') && (*(char *)(iVar10 + 8) != '\0')) &&
             ((bVar16 = *(byte *)(_p_llm_env + 0xd4), (param_1[0x15] & 0x10U) != 0 ||
              (((param_1[0x16] == '\0' && (4 < (byte)param_1[0x20])) &&
               (iVar10 = llm_scan_check_mesh_adv_data
                                   ((uint)(byte)param_1[0x28] + *(int *)(param_1 + 0x24)),
               iVar10 != 0)))))) {
            *(byte *)(iVar7 + 0xd4) = bVar16 | 0x40;
          }
          else {
            *(byte *)(iVar7 + 0xd4) = *(byte *)(iVar7 + 0xd4) & 0xbf;
            if (param_1[0x12] == '\0') {
              uVar15 = 0;
              uVar14 = 0xff;
            }
            else {
              uVar14 = *(ushort *)(param_1 + 0x10) >> 0xc;
              uVar15 = *(ushort *)(param_1 + 0x10) & 0xfff;
            }
            iVar12 = r_llm_adv_reports_list_check
                               (&uStack_60,uVar18,param_1[0x15],uVar14,uVar15,
                                *(undefined2 *)(param_1 + 0x1e));
          }
        }
        else if ((uVar6 & 3) == 2) {
          iVar12 = 1;
        }
        if (param_1[0x16] == '\x01') {
          iVar7 = (iVar12 != 0) + 1;
        }
        else {
          iVar7 = 0;
          if ((iVar12 == 0) && (iVar7 = 0, param_1[0x16] == '\x02')) {
            if (param_1[0x12] == '\0') {
              uVar15 = 0;
              uVar14 = 0xff;
            }
            else {
              uVar14 = *(ushort *)(param_1 + 0x10) >> 0xc;
              uVar15 = *(ushort *)(param_1 + 0x10) & 0xfff;
            }
            llm_adv_reports_list_remove
                      (&uStack_60,uVar18,param_1[0x15],uVar14,uVar15,*(undefined2 *)(param_1 + 0x1e)
                      );
            iVar7 = 0;
          }
        }
        if (bVar13 == 1) {
          bVar13 = *(byte *)(_p_llm_env + 0xd4) & 0xf3;
          bVar16 = (byte)(iVar7 << 2);
        }
        else {
          bVar13 = *(byte *)(_p_llm_env + 0xd4) & 0xcf;
          bVar16 = (byte)(iVar7 << 4);
        }
        *(byte *)(_p_llm_env + 0xd4) = bVar16 | bVar13;
        if (iVar12 != 0) goto _L530;
      }
      iVar12 = r_llm_adv_rep_flow_control_update_eco(param_1,&uStack_60);
      if (iVar12 == 0) {
        uVar17 = (undefined1)uVar18;
        if (*(char *)(_p_llm_env + 0xd7) == '\x01') {
          if ((((param_1[0x15] & 4U) == 0) || ((uStack_44._1_1_ & 0xc0) != 0x40)) || (bVar4 != 0xfe)
             ) {
            puVar9 = (undefined1 *)r_ke_msg_alloc(0x1104,0,0x3e,0x2b);
            *puVar9 = 2;
            puVar9[1] = 1;
            uVar2 = (&adv_evt_prop2type)[(byte)param_1[0x15]];
            puVar9[3] = uVar17;
            puVar9[2] = uVar2;
            memcpy(puVar9 + 4,&uStack_60,6);
            bVar4 = param_1[0x20];
            puVar9[10] = bVar4;
            memcpy(puVar9 + 0xb,(void *)((uint)(byte)param_1[0x28] + *(int *)(param_1 + 0x24)),
                   (uint)bVar4);
            memset(puVar9 + (byte)param_1[0x20] + 0xb,0,0x1f - (byte)param_1[0x20]);
            puVar9[0x2a] = param_1[0x1c];
          }
          else {
            puVar9 = (undefined1 *)r_ke_msg_alloc(0x1104,0,0x3e,0x12);
            *puVar9 = 0xb;
            puVar9[1] = 1;
            cVar1 = param_1[0x1c];
            puVar9[2] = 1;
            puVar9[0x11] = cVar1;
            puVar9[3] = uVar17;
            memcpy(puVar9 + 4,&uStack_60,6);
            puVar9[10] = 1;
            memcpy(puVar9 + 0xb,&uStack_48,6);
          }
          r_hci_send_2_host(puVar9);
        }
        else {
          uVar5 = (uint)(byte)param_1[0x20];
          uVar6 = (uint)(byte)param_1[0x28];
          do {
            __n = uVar5;
            if (0xe5 < uVar5) {
              __n = 0xe5;
            }
            puVar11 = (undefined2 *)r_ke_msg_alloc(0x1104,0,0x3e,0x100);
            *puVar11 = 0x10d;
            puVar11[1] = (ushort)(byte)param_1[0x15];
            if ((uVar5 < 0xe6) && ((param_1[0x16] & 0xfcU) != 0)) {
              r_assert_param(0x60,"llm_scan.c",0x7e2);
            }
            uVar18 = 0x20;
            if (uVar5 < 0xe6) {
              uVar18 = (ushort)(byte)param_1[0x16] << 5;
            }
            puVar11[1] = puVar11[1] & 0xff9f | uVar18;
            *(undefined1 *)(puVar11 + 2) = uVar17;
            memcpy((void *)((int)puVar11 + 5),&uStack_60,6);
            *(undefined *)((int)puVar11 + 0xb) = (&co_rate_to_phy)[(byte)param_1[0x19]];
            *(undefined *)(puVar11 + 6) = (&co_rate_to_phy)[(byte)param_1[0x1a]];
            bVar13 = 0xff;
            if (param_1[0x12] != '\0') {
              bVar13 = (byte)((ushort)*(undefined2 *)(param_1 + 0x10) >> 0xc);
            }
            *(byte *)((int)puVar11 + 0xd) = bVar13;
            *(char *)(puVar11 + 7) = param_1[0x1b];
            *(char *)((int)puVar11 + 0xf) =
                 (char)((int)*(short *)(_p_llm_env + 200) / 10) + param_1[0x1c];
            uVar3 = *(undefined2 *)(param_1 + 0x1e);
            *(byte *)(puVar11 + 9) = bVar4;
            puVar11[8] = uVar3;
            memcpy((void *)((int)puVar11 + 0x13),&uStack_48,6);
            *(char *)((int)puVar11 + 0x19) = (char)__n;
            memcpy(puVar11 + 0xd,(void *)(*(int *)(param_1 + 0x24) + uVar6),__n);
            memset((void *)(__n + 0x1a + (int)puVar11),0,0xe5 - __n);
            uVar5 = uVar5 - __n & 0xff;
            r_hci_send_2_host(puVar11);
            uVar6 = uVar6 + __n & 0xff;
          } while (uVar5 != 0);
        }
      }
    }
  }
_L530:
  if (param_1[0x20] != '\0') {
    r_ble_util_buf_rx_free(*(undefined2 *)(param_1 + 0x22),*(undefined4 *)(param_1 + 0x24));
  }
  return 0;
}


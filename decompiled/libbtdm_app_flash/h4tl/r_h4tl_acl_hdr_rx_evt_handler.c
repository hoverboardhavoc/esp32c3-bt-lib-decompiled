...

{
  ...
  if (_r_ke_free == 0) {
    r_hci_acl_tx_data_received(_DAT_00011016,0,0);
  ...
  _r_hci_cmd_received = (undefined2)iVar2;
  if (iVar2 == 0) {
    _r_rwip_prevent_sleep_clear = 0;
  }
  else {
    _r_rwip_prevent_sleep_clear = r_emi_get_mem_addr_by_offset();
    if (_r_rwip_prevent_sleep_clear != 0) {
      ...
      (*(code *)*_h4tl_env)
                (_r_rwip_prevent_sleep_clear,_r_ke_free,r_h4tl_rx_done,&h4tl_env,(code *)*_h4tl_env)
      ;
      return;
    }
  }

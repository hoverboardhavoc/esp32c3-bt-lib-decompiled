/*
 * Last changed at upstream commit b09bf658a78c1c234d5ba7b3174f0dca7dd80c6b
 * https://github.com/espressif/esp32c3-bt-lib/commit/b09bf658a78c1c234d5ba7b3174f0dca7dd80c6b
 * Upstream date: 2025-04-28 11:55:39 +0800
 * Upstream subject: fix(bt): Update bt lib for ESP32-C3 and ESP32-S3(edf923e)
 * Source: libbtdm_app_flash -> lld_per_adv.o -> r_lld_per_adv_start
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

int r_lld_per_adv_start(uint param_1,ushort *param_2)

{
  ushort uVar1;
  undefined2 uVar2;
  undefined1 uVar3;
  char cVar4;
  uint uVar5;
  uint *puVar6;
  void *__s;
  byte *pbVar7;
  undefined4 uVar8;
  int iVar9;
  byte bVar10;
  uint uVar11;
  int iVar12;
  uint uVar13;
  byte bStack_32;
  byte abStack_31 [5];
  
  iVar12 = 0xc;
  if (*(int *)(&lld_per_adv_env + param_1 * 4) == 0) {
    __s = (void *)r_ke_malloc(0x60,0);
    *(void **)(&lld_per_adv_env + param_1 * 4) = __s;
    if (__s == (void *)0x0) {
      r_assert_err(0x10000,0x587);
    }
    else {
      bStack_32 = 0;
      abStack_31[0] = 0;
      memset(__s,0,0x60);
      *(code **)((int)__s + 0x20) = r_lld_per_adv_evt_canceled_cbk;
      *(code **)((int)__s + 0x18) = r_lld_per_adv_evt_start_cbk;
      uVar3 = DAT_0001301a;
      *(char *)((int)__s + 0x52) = (char)param_1;
      *(undefined1 *)((int)__s + 0x16) = uVar3;
      *(ushort *)((int)__s + 0x48) = param_2[1];
      *(uint *)((int)__s + 0x40) = (uint)*param_2 << 2;
      *(undefined2 *)((int)__s + 0x3a) = 10;
      *(char *)((int)__s + 0x54) = (char)param_2[5];
      *(undefined1 *)((int)__s + 0x56) = *(undefined1 *)((int)param_2 + 0xb);
      bVar10 = (byte)param_2[6];
      if ((bVar10 < 3) || (*(char *)(_p_lld_env + 0xda) == '\0')) {
        bVar10 = bVar10 - 1;
      }
      *(byte *)((int)__s + 0x55) = bVar10;
      iVar12 = _p_lld_env;
      *(ushort *)((int)__s + 0x50) = param_2[3];
      uVar1 = param_2[2];
      *(undefined1 *)((int)__s + 0x2f) = 0;
      *(undefined1 *)((int)__s + 0x30) = 0;
      *(ushort *)((int)__s + 0x4e) = uVar1;
      *(undefined1 *)((int)__s + 0x31) = 0;
      *(undefined1 *)((int)__s + 0x32) = 0;
      *(undefined1 *)((int)__s + 0x33) = 0;
      iVar9 = (uint)*(byte *)(iVar12 + 0x100) + iVar12;
      for (; iVar12 != iVar9; iVar12 = iVar12 + 1) {
        bVar10 = *(byte *)(iVar12 + 0xdb) >> 3;
        *(byte *)((int)__s + bVar10 + 0x2f) =
             (byte)(1 << (*(byte *)(iVar12 + 0xdb) & 7)) | *(byte *)((int)__s + bVar10 + 0x2f);
      }
      r_lld_per_adv_chain_construct(param_1);
      r_bt_rma_get_ant_by_act(6,param_1,&bStack_32,abStack_31);
      uVar11 = (uint)rwip_coex_cfg;
      uVar5 = (uint)bStack_32;
      uVar13 = (uint)abStack_31[0];
      if ((uVar5 << 7 & 0xffffff7f) != 0) {
        r_assert_err(0,0x10000,0x42);
      }
      if ((uVar13 << 6 & 0xffffffbf) != 0) {
        r_assert_err(0,0x10000,0x43);
      }
      iVar9 = param_1 * 0x5a;
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(ushort *)(iVar12 + iVar9) =
           (ushort)(uVar5 << 7) | (ushort)(uVar13 << 6) | (ushort)((uVar11 & 4) << 6) |
           (ushort)((uVar11 & 1) << 10) | (ushort)((uVar11 & 2) << 8) | 6;
      pbVar7 = (byte *)r_bt_rf_coex_st_param_get(4);
      if (pbVar7 != (byte *)0x0) {
        bVar10 = pbVar7[1];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x299b);
        }
        if (((uint)bVar10 << 8 & 0xf000) != 0) {
          r_assert_err(0,0x10000,0x299d);
        }
        puVar6 = (uint *)((param_1 + 0x1800c4cb) * 4);
        *puVar6 = *puVar6 & 0xfffff0ff | (uint)bVar10 << 8;
        bVar10 = *pbVar7;
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x294a);
        }
        *puVar6 = (uint)bVar10 << 0x1e | *puVar6 & 0xbfffffff;
        bVar10 = pbVar7[3];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x29bb);
        }
        if ((bVar10 & 0xf0) != 0) {
          r_assert_err(0,0x10000,0x29bd);
        }
        *puVar6 = *puVar6 & 0xfffffff0 | (uint)bVar10;
        bVar10 = pbVar7[2];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x295a);
        }
        *puVar6 = (uint)bVar10 << 0x1d | *puVar6 & 0xdfffffff;
        bVar10 = pbVar7[7];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x298b);
        }
        if (((uint)bVar10 << 0xc & 0xf0000) != 0) {
          r_assert_err(0,0x10000,0x298d);
        }
        *puVar6 = *puVar6 & 0xffff0fff | (uint)bVar10 << 0xc;
        bVar10 = pbVar7[8];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x29ab);
        }
        if (((uint)bVar10 << 4 & 0xffffff0f) != 0) {
          r_assert_err(0,0x10000,0x29ad);
        }
        *puVar6 = *puVar6 & 0xffffff0f | (uint)bVar10 << 4;
        bVar10 = pbVar7[9];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x297b);
        }
        if (((uint)bVar10 << 0x10 & 0xf00000) != 0) {
          r_assert_err(0,0x10000,0x297d);
        }
        *puVar6 = *puVar6 & 0xfff0ffff | (uint)bVar10 << 0x10;
        uVar1 = *(ushort *)(pbVar7 + 10);
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x296a);
        }
        if (((uint)uVar1 << 0x14 & 0xe0000000) != 0) {
          r_assert_err(0,0x10000,0x296c);
        }
        *puVar6 = *puVar6 & 0xe00fffff | (uint)uVar1 << 0x14;
        bVar10 = pbVar7[6];
        if (0xb < param_1) {
          r_assert_err(0,0x10000,0x293a);
        }
        *puVar6 = *puVar6 & 0x7fffffff | (uint)bVar10 << 0x1f;
        if (2 < (int)_g_bt_plf_log_level) {
          ets_printf("PTI CNTL CS[%d] REG[%08x] TYPE[PER_ADV] RXEN[%d] RX[%d] IFSEN[%d] IFS[%d] PRTEN[%d] TIME[%d] DIFS[%d] DRX[%d] DTX[%d]\n"
                     ,param_1,puVar6,pbVar7[2],pbVar7[3],*pbVar7,pbVar7[1],pbVar7[6]);
        }
      }
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x16 + iVar12) = 0xc027;
      if (*(char *)((int)__s + 0x56) != '\x7f') {
        uVar8 = (*_g_bt_plf_log_level)(0,_g_bt_plf_log_level);
        r_bt_rtp_register_rule_cs_idx(param_1,uVar8);
      }
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x18 + iVar12) = 0;
      r_lld_per_adv_set_tx_power(param_1,6);
      uVar2 = *(undefined2 *)((int)__s + 0x2f);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x22 + iVar12) = uVar2;
      uVar2 = *(undefined2 *)((int)__s + 0x31);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x24 + iVar12) = uVar2;
      bVar10 = *(byte *)((int)__s + 0x33);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(ushort *)(iVar9 + 0x26 + iVar12) = (ushort)bVar10;
      r_lld_aa_gen((int)__s + 0x28,param_1);
      uVar8 = (**(code **)(_r_osi_funcs_p + 0x88))(*(code **)(_r_osi_funcs_p + 0x88));
      *(short *)((int)__s + 0x2c) = (short)uVar8;
      *(char *)((int)__s + 0x2e) = (char)((uint)uVar8 >> 0x10);
      uVar2 = *(undefined2 *)((int)__s + 0x28);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0xc + iVar12) = uVar2;
      uVar2 = *(undefined2 *)((int)__s + 0x2a);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0xe + iVar12) = uVar2;
      uVar2 = *(undefined2 *)((int)__s + 0x2c);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x10 + iVar12) = uVar2;
      bVar10 = *(byte *)((int)__s + 0x2e);
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(ushort *)(iVar9 + 0x12 + iVar12) = (ushort)bVar10;
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(undefined2 *)(iVar9 + 0x28 + iVar12) = 0;
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      uVar5 = *(uint *)((int)__s + 0x44);
      *(undefined2 *)(iVar9 + 0x2a + iVar12) = 0;
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(short *)(iVar9 + 0x20 + iVar12) = (short)((uVar5 / 0x271 + 1) * 0x10000 >> 0x10);
      bVar10 = *(byte *)((int)__s + 0x55);
      uVar5 = (uint)bVar10 << 4;
      if ((uVar5 & 0xffffffcf) != 0) {
        r_assert_err(0,0x10000,0x18e);
      }
      uVar11 = (uint)bVar10 << 2;
      if ((uVar11 & 0xfffffff3) != 0) {
        r_assert_err(0,0x10000,399);
      }
      if ((bVar10 & 0xfc) != 0) {
        r_assert_err(0,0x10000,400);
      }
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(ushort *)(iVar9 + 4 + iVar12) = (ushort)uVar5 | (ushort)uVar11 | (ushort)bVar10;
      uVar1 = param_2[5];
      if ((param_1 & 0xffffffe0) != 0) {
        r_assert_err(0,0x10000,0xe6);
      }
      iVar12 = r_emi_get_mem_addr_by_offset(0x400);
      *(ushort *)(iVar9 + 2 + iVar12) = (ushort)(((byte)uVar1 & 1) << 7) | (ushort)param_1 | 0x800;
      iVar12 = r_lld_read_clock();
      uVar11 = *(uint *)((int)__s + 0x40);
      uVar5 = rwip_prog_delay + 1 + iVar12 & 0xfffffff;
      if (uVar11 == 0) {
        r_assert_err(0,0x10000,0x94);
      }
      bVar10 = DAT_0001301b;
      uVar1 = param_2[4];
      *(undefined4 *)((int)__s + 8) = 0;
      *(uint *)((int)__s + 4) = ((uint)uVar1 * 2 + uVar11 + uVar5) - uVar5 % uVar11 & 0xfffffff;
      *(ushort *)((int)__s + 0x14) = bVar10 & 0xf | 0x2000;
      cVar4 = '\x0f';
      do {
        iVar12 = r_sch_arb_insert(__s);
        if (iVar12 == 0) {
          *(undefined1 *)((int)__s + 0x53) = 0;
          goto _L332;
        }
        cVar4 = cVar4 + -1;
        *(byte *)((int)__s + 0x16) = *(char *)((int)__s + 0x16) + bVar10;
        *(uint *)((int)__s + 4) = *(int *)((int)__s + 4) + *(int *)((int)__s + 0x40) & 0xfffffff;
      } while (cVar4 != '\0');
      r_assert_err(0,0x10000,0x57d);
_L332:
      r_lld_per_adv_coex_env_reset(__s);
      iVar12 = 0;
    }
  }
  iVar9 = r_sdk_config_get_opts_ext();
  if ((*(uint *)(iVar9 + 0x28) & 0x40) != 0) {
    iVar9 = r_sdk_config_get_opts_ext();
    if (*(byte *)(iVar9 + 0x2c) < 3) {
      r_ble_log_internal_x2(0x40080000,iVar12 << 8 | param_1,*param_2);
    }
  }
  return iVar12;
}


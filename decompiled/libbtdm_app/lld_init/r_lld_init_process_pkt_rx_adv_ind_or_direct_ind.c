/*
 * Last changed at upstream commit a3f7d7ffa2b261b1415042d8b7cd457cc2b4b1de
 * https://github.com/espressif/esp32c3-bt-lib/commit/a3f7d7ffa2b261b1415042d8b7cd457cc2b4b1de
 * Upstream date: 2021-12-28 18:10:18 +0800
 * Upstream subject: Update ESP32-C3 and ESP32-S3 bt lib (33175c8)
 * Source: libbtdm_app -> lld_init.o -> r_lld_init_process_pkt_rx_adv_ind_or_direct_ind
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void r_lld_init_process_pkt_rx_adv_ind_or_direct_ind(int param_1,undefined4 param_2)

{
  byte bVar1;
  ushort uVar2;
  ushort uVar3;
  ushort uVar4;
  undefined2 uVar5;
  ushort uVar6;
  int iVar7;
  uint uVar8;
  int iVar9;
  int iVar10;
  int iVar11;
  void *__src;
  int iVar12;
  int iVar13;
  
  iVar7 = lld_init_env;
  bVar1 = *(byte *)(_p_lld_env + 0xd8);
  iVar13 = *(int *)(param_1 * 4 + lld_init_env);
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  iVar9 = (uint)bVar1 * 0x14;
  iVar12 = iVar9 + 4;
  if ((*(ushort *)(iVar10 + iVar12) & 0xe) != 0) {
                    /* WARNING: Could not recover jumptable at 0x00011a28. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    (**(code **)(_r_plf_funcs_p + 8))(0,"lld_init.c",0x254);
    return;
  }
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar2 = *(ushort *)(iVar10 + iVar12);
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar3 = *(ushort *)(iVar10 + iVar9 + 6);
  uVar8 = (uint)(uVar3 >> 0xe);
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar4 = *(ushort *)(iVar10 + iVar9 + 10);
  if ((uVar4 & 0xf000) != 0) {
    (**(code **)(_r_plf_funcs_p + 8))(0,"lld_init.c",0x3e5,*(code **)(_r_plf_funcs_p + 8));
  }
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar5 = *(undefined2 *)(iVar10 + iVar9 + 8);
  bVar1 = *(byte *)(_p_lld_env + 0xd8);
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar6 = *(ushort *)((uint)bVar1 * 0x14 + 0xc + iVar10);
  iVar10 = (**(code **)(_r_modules_funcs_p + 4))
                     (uVar2 >> 8,uVar8,*(code **)(_r_modules_funcs_p + 4));
  uVar2 = *(ushort *)(&lld_exp_sync_pos_tab + uVar8 * 2);
  iVar11 = (**(code **)(_r_modules_funcs_p + 4))(0x22,uVar8,*(code **)(_r_modules_funcs_p + 4));
  *(uint *)(iVar7 + 0x54) =
       (((iVar11 + (iVar10 - (uint)uVar2) + 0x96) * 2 - (uVar6 & 0x3ff)) + 0x4e0) / 0x271 +
       CONCAT22(uVar4,uVar5) & 0xfffffff;
  iVar10 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  *(undefined2 *)(iVar7 + 0x4c) = *(undefined2 *)(iVar9 + 0xe + iVar10);
  iVar9 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar2 = *(ushort *)(iVar9 + iVar12);
  *(byte *)(iVar7 + 0x4e) = (byte)(uVar3 >> 0xe);
  *(byte *)(iVar7 + 0x58) = (byte)(uVar2 >> 5) & 1;
  __src = (void *)(**(code **)(_r_plf_funcs_p + 0xbc))(param_2,*(code **)(_r_plf_funcs_p + 0xbc));
  memcpy((void *)(iVar13 + 0x32),__src,6);
  iVar9 = (**(code **)(_r_plf_funcs_p + 0xbc))(0x1000,*(code **)(_r_plf_funcs_p + 0xbc));
  uVar2 = *(ushort *)(iVar9 + iVar12);
  *(undefined1 *)(iVar7 + 0x59) = 0;
  *(byte *)(iVar7 + 0x33) = (byte)(uVar2 >> 6) & 1;
  *(undefined1 *)(iVar7 + 0x50) = 1;
  return;
}


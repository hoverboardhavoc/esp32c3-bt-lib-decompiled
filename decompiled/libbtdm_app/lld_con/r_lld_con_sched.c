/*
 * Last changed at upstream commit db872ab1620e1656f51d7a69c5a0576a6f369501
 * https://github.com/espressif/esp32c3-bt-lib/commit/db872ab1620e1656f51d7a69c5a0576a6f369501
 * Upstream date: 2025-04-23 17:25:53 +0800
 * Upstream subject: fix(bt): Update bt lib for ESP32-C3 and ESP32-S3(edf923e)
 * Source: libbtdm_app -> lld_con.o -> r_lld_con_sched
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void r_lld_con_sched(int param_1,int param_2,int param_3)

{
  ushort uVar1;
  bool bVar2;
  int iVar3;
  uint uVar4;
  uint uVar5;
  ushort uVar6;
  short sVar7;
  int iVar8;
  uint uVar9;
  uint uVar10;
  uint uVar11;
  uint uVar12;
  undefined2 uVar13;
  int iVar14;
  
  iVar3 = *(int *)(&lld_con_env + param_1 * 4);
  uVar6 = *(ushort *)(iVar3 + 0x84);
  uVar10 = *(uint *)(iVar3 + 0x48);
  uVar4 = (uint)DAT_00016026 << 1;
  if ((uVar6 & 2) == 0) {
    *(undefined1 *)(iVar3 + 0x16) = 0x1f;
  }
  else if (((uVar6 & 3) == 3) && (*(undefined1 *)(iVar3 + 0x96) = 0, param_3 != 0)) {
    uVar1 = *(ushort *)(iVar3 + 0x72);
    if ((uVar1 != 0) &&
       ((((uVar6 & 0x404) == 0x400 && ((uVar6 & 8) == 0)) && (*(char *)(iVar3 + 0x46) == '\0')))) {
      uVar10 = (uint)uVar1 * *(int *)(iVar3 + 100) + uVar10 & 0xfffffff;
      *(ushort *)(iVar3 + 0x7e) = uVar1 + *(short *)(iVar3 + 0x7e);
      *(undefined1 *)(iVar3 + 0x96) = 1;
    }
  }
  for (; (param_2 - uVar10 & 0xfffffff) < 0x7ffffff;
      uVar10 = *(int *)(iVar3 + 100) + uVar10 & 0xfffffff) {
    *(short *)(iVar3 + 0x7e) = *(short *)(iVar3 + 0x7e) + 1;
  }
  uVar12 = *(uint *)(iVar3 + 0x58);
  if (((*(char *)(iVar3 + 0x46) != '\x01') || (*(char *)(iVar3 + 0x43) != '\0')) ||
     (0x7ffe < (ushort)((*(short *)(iVar3 + 0x7c) + *(short *)(iVar3 + 0x7e)) -
                       *(short *)(iVar3 + 0x44)))) goto _L34;
  if (*(char *)(iVar3 + 0x42) == '\0') {
    uVar10 = *(ushort *)(iVar3 + 0x3c) + uVar10 & 0xfffffff;
    if ((uVar6 & 1) != 0) {
      uVar10 = (*(ushort *)(iVar3 + 0x40) >> 1) + uVar10 & 0xfffffff;
    }
    *(undefined4 *)(iVar3 + 100) = *(undefined4 *)(iVar3 + 0x34);
    *(undefined2 *)(iVar3 + 0x72) = *(undefined2 *)(iVar3 + 0x3e);
    *(undefined4 *)(iVar3 + 0x68) = *(undefined4 *)(iVar3 + 0x38);
    *(undefined1 *)(iVar3 + 0x42) = 1;
    if ((uVar6 & 1) != 0) {
      (**(code **)(_r_ip_funcs_p + 0x36c))
                (*(undefined1 *)(iVar3 + 0x8e),*(code **)(_r_ip_funcs_p + 0x36c));
      uVar6 = *(ushort *)(iVar3 + 0x84);
      if ((uVar6 & 1) != 0) goto _L35;
    }
    *(undefined1 *)(iVar3 + 0x43) = 1;
  }
_L35:
  sVar7 = (*(short *)(iVar3 + 0x7c) + *(short *)(iVar3 + 0x7e)) - *(short *)(iVar3 + 0x44);
  uVar12 = uVar10;
  while (sVar7 = sVar7 + -1, sVar7 != -1) {
    uVar12 = uVar12 - *(int *)(iVar3 + 100) & 0xfffffff;
  }
_L34:
  if ((uVar6 & 2) == 0) {
    uVar11 = *(int *)(iVar3 + 100) * 6;
  }
  else {
    uVar11 = *(uint *)(iVar3 + 0x68);
  }
  iVar14 = 0;
  do {
    uVar13 = (undefined2)iVar14;
    if (uVar11 < (uVar10 - uVar12 & 0xfffffff)) {
      bVar2 = false;
_L52:
      if (!bVar2) {
                    /* WARNING: Could not recover jumptable at 0x000105b8. Too many branches */
                    /* WARNING: Treating indirect jump as call */
        (**(code **)(_r_ip_funcs_p + 800))
                  (param_1,1,(-((*(ushort *)(iVar3 + 0x84) & 2) == 0) & 0x36U) + 8,
                   *(code **)(_r_ip_funcs_p + 800));
        return;
      }
      *(undefined1 *)(iVar3 + 0x8f) = 0;
      *(uint *)(iVar3 + 0x48) = uVar10;
      if ((*(ushort *)(iVar3 + 0x84) & 1) != 0) {
        *(undefined2 *)(iVar3 + 0x6c) = uVar13;
        *(uint *)(iVar3 + 0x4c) = (uint)DAT_00016026 * 2 + uVar4;
      }
      return;
    }
    uVar9 = uVar10;
    if ((*(ushort *)(iVar3 + 0x84) & 1) != 0) {
      iVar8 = *(int *)(iVar3 + 0x50);
      sVar7 = *(short *)(iVar3 + 0x6e);
      iVar14 = (**(code **)(_r_modules_funcs_p + 0x28c))(*(code **)(_r_modules_funcs_p + 0x28c));
      uVar4 = 0x14;
      if (iVar14 == 0) {
        uVar4 = (uint)*(ushort *)(_p_lld_env + 0xd4);
      }
      uVar6 = *(ushort *)(iVar3 + 0x7a);
      iVar14 = (**(code **)(_r_ip_funcs_p + 0x91c))(*(code **)(_r_ip_funcs_p + 0x91c));
      uVar4 = (((uVar6 + uVar4) * (uVar10 - iVar8 & 0xfffffff)) / 0x640 + 0x40) * 2;
      if (param_3 != 0) {
        *(undefined2 *)(iVar14 + (param_1 + 0x10) * 2 + 8) = 0;
      }
      if ((*(ushort *)(iVar3 + 0x84) & 2) == 0) {
        uVar4 = uVar4 + (uint)*(ushort *)(iVar3 + 0x70) * 0x271;
      }
      else if ((*(ushort *)(iVar3 + 0x7c) < 7) &&
              ((param_3 == 0 ||
               ((*(ushort *)(iVar3 + 0x7c) == 0 && (*(short *)(iVar3 + 0x98) == -1)))))) {
        iVar14 = iVar14 + param_1 * 2;
        uVar6 = *(ushort *)(iVar14 + 0x28);
        uVar4 = uVar4 + ((int)((uint)uVar6 * (uint)*(ushort *)(iVar3 + 0x70) * 0x271) >> 1) +
                        (uint)*(ushort *)(iVar3 + 0x70) * 0x271;
        uVar9 = *(int *)(iVar3 + 100) * 0x271;
        if (uVar9 < uVar4) {
          uVar4 = uVar9;
        }
        *(ushort *)(iVar14 + 0x28) = uVar6 + 1;
      }
      if (((*(char *)(iVar3 + 0x46) == '\x01') && (*(char *)(iVar3 + 0x42) != '\0')) &&
         (*(char *)(iVar3 + 0x43) == '\0')) {
        uVar4 = uVar4 + (uint)*(ushort *)(iVar3 + 0x40) * 0x271;
      }
      uVar5 = (uVar4 >> 1) / 0x271;
      uVar9 = uVar10 - uVar5 & 0xfffffff;
      iVar14 = ((int)sVar7 - (uVar4 >> 1)) + uVar5 * 0x271;
      if (iVar14 < 0) {
        uVar9 = uVar9 - 1 & 0xfffffff;
        iVar14 = iVar14 + 0x271;
      }
      *(int *)(iVar3 + 8) = iVar14;
      *(uint *)(iVar3 + 0x10) = *(int *)(iVar3 + 0x5c) + uVar4;
    }
    uVar13 = (undefined2)iVar14;
    *(uint *)(iVar3 + 4) = uVar9;
    iVar8 = (**(code **)(_r_ip_funcs_p + 0x6b0))(iVar3,*(code **)(_r_ip_funcs_p + 0x6b0));
    if (iVar8 == 0) {
      bVar2 = true;
      goto _L52;
    }
    *(char *)(iVar3 + 0x16) = *(char *)(iVar3 + 0x16) + rwip_priority;
    uVar10 = *(int *)(iVar3 + 100) + uVar10 & 0xfffffff;
    sVar7 = *(short *)(iVar3 + 0x7e);
    *(short *)(iVar3 + 0x7e) = sVar7 + 1;
    if (((*(char *)(iVar3 + 0x46) == '\x01') && (*(char *)(iVar3 + 0x43) == '\0')) &&
       ((ushort)(((sVar7 + 1) - *(short *)(iVar3 + 0x44)) + *(short *)(iVar3 + 0x7c)) < 0x7fff)) {
      if (*(char *)(iVar3 + 0x42) == '\0') {
        uVar10 = *(ushort *)(iVar3 + 0x3c) + uVar10 & 0xfffffff;
        if ((*(ushort *)(iVar3 + 0x84) & 1) != 0) {
          uVar10 = (*(ushort *)(iVar3 + 0x40) >> 1) + uVar10 & 0xfffffff;
        }
        *(undefined4 *)(iVar3 + 100) = *(undefined4 *)(iVar3 + 0x34);
        *(undefined2 *)(iVar3 + 0x72) = *(undefined2 *)(iVar3 + 0x3e);
        *(undefined4 *)(iVar3 + 0x68) = *(undefined4 *)(iVar3 + 0x38);
        if ((*(ushort *)(iVar3 + 0x84) & 1) != 0) {
          (**(code **)(_r_ip_funcs_p + 0x36c))
                    (*(undefined1 *)(iVar3 + 0x8e),*(code **)(_r_ip_funcs_p + 0x36c));
        }
        *(undefined1 *)(iVar3 + 0x42) = 1;
        uVar11 = *(uint *)(iVar3 + 0x68);
        if ((*(ushort *)(iVar3 + 0x84) & 1) == 0) {
          *(undefined1 *)(iVar3 + 0x43) = 1;
        }
      }
      sVar7 = (*(short *)(iVar3 + 0x7c) + *(short *)(iVar3 + 0x7e)) - *(short *)(iVar3 + 0x44);
      uVar12 = uVar10;
      while (sVar7 = sVar7 + -1, sVar7 != -1) {
        uVar12 = uVar12 - *(int *)(iVar3 + 100) & 0xfffffff;
      }
    }
  } while( true );
}

